...
fn main() {
    ...
    let activation_fn: &dyn Fn(&f64) -> f64 = &(|x| sigmoid(*x, 1.0));
    let result = neural_network.feed_forward(training_images[0].clone(), activation_fn);
    println!("result: {:?} \nlabel: {}", result, training_labels[0]);

    let pixel_data: Vec<u8> = training_images[0].iter().map(|x| ((1.0-x)*256.0) as u8).collect();
